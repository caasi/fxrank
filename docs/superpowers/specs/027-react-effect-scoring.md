...
Today component detection is `returns_jsx`, which misses `return null` components. Recognize a component by **any** of: PascalCase function (declared or default-exported) in a `.tsx`/`.jsx`-dialect file; a function used as a JSX element; a default export of a component-file; a function that calls React hooks. TS-only heuristic, carries `confidence` (lower when only one weak signal holds). Non-components keep today's per-function scoring. **Recognition does NOT change the `<module>` synthetic unit's scope** (`module_init_unit` still scores only top-level statements): a hook-calling helper or a default-exported component must not have its effects counted both in the component AND in `<module>` — re-parenting (§4.2) assigns each effect exactly one owner, and a component's body effects never belong to `<module>`.
...
- **`EscapedValue`** — returned/exported, stored in module/global/context/ref, or passed to an unknown callee. **Not** adopted; left standalone + graph edge.
