...
- **Descendant-local named handlers** — a named function declared inside an owned descendant callback and referenced there (e.g. `onClick={inner}` inside a render-prop) resolves via the component-scoped provenance table as `Unknown`, so it is adopted at the descendant's phase rather than its use-site phase: a ≤1-class over-score (no event-phase conditionality discount) and possibly a spurious `effect.in.render`. Bounded and over-conservative (never hides an effect, only over-attributes risk); deferred (precise fix would need descendant-scoped provenance, which decl-site fallback alone cannot provide for a descendant-local decl).
